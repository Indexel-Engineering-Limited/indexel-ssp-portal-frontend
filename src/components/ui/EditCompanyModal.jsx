...
export default function EditCompanyModal({ company, onClose, onSave }) {
  ...
  // On initial mount we do NOT wipe state/city so the
  // pre-filled values are preserved while options load.
  // --------------------------------------------------
  ...
  useEffect(() => {
    ...
    // Only reset state/city when the user actively changes the country
    if (!isInitial) {
      setStates([]);
      setCities([]);
  ...
  // Same pattern: skip reset on initial mount.
  // --------------------------------------------------
  const initialStateMountRef = useRef(true);
  ...
  useEffect(() => {
    ...
    const isInitial = initialStateMountRef.current;
    initialStateMountRef.current = false;

    let cancelled = false;

    // Only reset city when the user actively changes the state
    if (!isInitial) {
      setCities([]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      await onSave(company.company_id, {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg bg-[#f8fafd] rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto mx-4">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#d4e0f0]/60 sticky top-0 bg-[#f8fafd] z-10">
          <div><h2 className="text-[16px] font-semibold text-[#111827]">Edit Company</h2><p className="text-[11.5px] text-[#6b7280] mt-0.5">{company.company_name}</p></div>
